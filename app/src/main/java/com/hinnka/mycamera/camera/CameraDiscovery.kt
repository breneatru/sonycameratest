...
class CameraDiscovery(private val context: Context) {
    ...
    companion object {
        ...
        // 需要跳过探测的厂商
        private val SKIP_PROBE_MANUFACTURERS = setOf("huawei", "honor", "sony")
    ...
    fun discoverAllCameras(): List<CameraInfo> {
        ...
        // Sony-specific: handle variable telephoto (70mm/105mm)
        val sonyOptimizedCameras = if (DeviceUtil.isSony) {
            optimizeSonyVariableTelephoto(classifiedBackCameras)
        } else {
            classifiedBackCameras
        }
        
        cameras.addAll(sonyOptimizedCameras)
    ...
    /**
     * Sony-specific optimization for variable telephoto (70mm/105mm)
     * Xperia 1 Mark 3 has a unique periscope telephoto that can switch between 70mm and 105mm
     */
    private fun optimizeSonyVariableTelephoto(cameras: List<CameraInfo>): List<CameraInfo> {
        val result = mutableListOf<CameraInfo>()
        
        // Find telephoto cameras (intrinsicZoomRatio > 2.0)
        val telephotoCameras = cameras.filter { 
            it.lensType == LensType.BACK_TELEPHOTO && it.intrinsicZoomRatio > 2.0f 
        }
        
        if (telephotoCameras.isEmpty()) {
            return cameras
        }
        
        // Check if we have a telephoto with variable focal length (Sony unique feature)
        // Sony Xperia 1 Mark 3 telephoto has intrinsicZoomRatio ~2.9 (70mm) and ~4.4 (105mm)
        val variableTelephoto = telephotoCameras.find { 
            it.intrinsicZoomRatio >= 2.8f && it.intrinsicZoomRatio <= 3.0f 
        }
        
        if (variableTelephoto != null) {
            PLog.d(TAG, "Detected Sony variable telephoto: ${variableTelephoto.cameraId}, intrinsicZoom=${variableTelephoto.intrinsicZoomRatio}")
            
            // Create virtual entries for 70mm and 105mm modes
            val tele70mm = variableTelephoto.copy(
                cameraId = "${variableTelephoto.cameraId}_70mm",
                lensType = LensType.BACK_TELEPHOTO,
                intrinsicZoomRatio = 2.92f, // 70mm equivalent
                displayIntrinsicZoomRatio = 2.92f,
                focalLength35mmEquivalent = 70f,
                isVirtualIszLens = true,
                baseCameraId = variableTelephoto.cameraId,
                iszZoomRatio = 1.0f
            )
            
            val tele105mm = variableTelephoto.copy(
                cameraId = "${variableTelephoto.cameraId}_105mm",
                lensType = LensType.BACK_TELEPHOTO,
                intrinsicZoomRatio = 4.38f, // 105mm equivalent
                displayIntrinsicZoomRatio = 4.38f,
                focalLength35mmEquivalent = 105f,
                isVirtualIszLens = true,
                baseCameraId = variableTelephoto.cameraId,
                iszZoomRatio = 1.5f
            )
            
            // Replace original with two virtual entries
            result.addAll(cameras.filter { it.cameraId != variableTelephoto.cameraId })
            result.add(tele70mm)
            result.add(tele105mm)
            
            PLog.d(TAG, "Added Sony variable telephoto modes: 70mm (2.92x) and 105mm (4.38x)")
        } else {
